...
userHandles
  .get('/', (req, resp, next) => {
    ...
    resp.set(200, {'Content-Type': 'text/html'})
    resp.send(content)
  ...
  .get('/hello', (req, resp, next) => { 
    ...
    res.send("To see a Hello message: /hello?name='NAME' !" + '<br/>')
  ...
  .get('/about', (req, resp, next) => { 
    ...
    for(var attributename in about){
      resp.send(attributename+": "+ about[attributename] + '\n');
    }
  ...
  .get('/*/', (req, resp, next) => { 
    ...
    if(fs.existsSync("./content" + path + '.json') )
    {
      ...
      for(var attributename in json){
        res.send(attributename+": "+json[attributename] + '\n');
      }
    }
    else
    {
